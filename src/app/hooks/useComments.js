...
const useComments = () => {
    ...
    const fetchComments = async (postId) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URI}comments/${postId}`,{
                cache: "no-cache",
            });
            if(!res.ok) {
    ...
    const handleUpdate = async (commentId,postId) => {
        try {
            setIsLoading(true);
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URI}comments/${commentId}`,{
                method: 'PATCH',
                headers: {
            ...
            setIsLoading(false);
            refreshComments(postId); // refetch or update comment state locally
            cancelEdit();
        } catch (err) {
