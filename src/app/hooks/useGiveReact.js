...
const useGiveReact = () => {
    ...
    const getReactionIcon = (type, isActive) => {
        const baseClass = "text-[18px] cursor-pointer transition-transform duration-150";
...
try {
        setIsReacted(true)
    const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URI}reactions/${selectedReaction?.blogId}`, {
        method: 'POST',
        headers: {
